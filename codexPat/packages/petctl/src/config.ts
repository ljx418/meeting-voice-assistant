...
export function resolveUrl(cliUrl?: string): Resolution {
  if (cliUrl) {
    return { value: trimTrailingSlash(cliUrl), source: "cli" };
  }
...
export function resolveToken(cliToken?: string): Resolution {
  if (cliToken) {
    return { value: cliToken, source: "cli" };
  }
  if (process.env.AGENT_DESKTOP_PET_TOKEN) {
    return { value: process.env.AGENT_DESKTOP_PET_TOKEN, source: "env" };
  }
