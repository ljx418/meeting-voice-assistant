...
`docs/design/V2.0/harnessos_architecture_master_spec_v2.md` 已被采纳为下一版正式目标架构主干。该设计与当前 Baseline v1.5-E 方向一致，但定位更明确：harnessOS 的最终目标不是单一助手，也不只是当前 Gateway/Workflow 集合，而是 **Protocol-first Harness Core + OS-like Agent App Server + Domain Pack Platform**。
