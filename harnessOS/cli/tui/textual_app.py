...
from harnessOS.openharness.api.client import SupportsStreamingMessages
from harnessOS.openharness.config.settings import load_settings, save_settings
from harnessOS.openharness.engine.stream_events import (
    AssistantTextDelta,
...
from harnessOS.openharness.ui.runtime import build_runtime, close_runtime, handle_line, start_runtime
...
class OpenHarnessTerminalApp(App[None]):
    ...
    async def on_mount(self) -> None:
        import os
        log_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "tui_debug.log")
        def log(msg):
            with open(log_path, "a") as f:
                f.write(f"{msg}\n")
        log(f"[DEBUG] on_mount called, bundle is {self._bundle}")
        try:
            ...
            log(f"[DEBUG] build_runtime done, bundle={self._bundle}")
            await start_runtime(self._bundle)
            log(f"[DEBUG] start_runtime done")
            ...
            log(f"[DEBUG] on_mount complete")
        except Exception as e:
            log(f"[DEBUG] on_mount error: {e}")
            import traceback
            with open(log_path, "a") as f:
    ...
    async def handle_submit(self, event: Input.Submitted) -> None:
        ...
        def log(msg):
            with open(log_path, "a") as f:
                f.write(f"{msg}\n")
        log(f"[DEBUG] handle_submit called, value={event.value!r}")
        event.input.value = ""
    ...
    async def _process_line(self, line: str) -> None:
        ...
        def log(msg):
            with open(log_path, "a") as f:
                f.write(f"{msg}\n")
        log(f"[DEBUG] _process_line called with: {line!r}")
        log(f"[DEBUG] _busy={self._busy}, bundle={self._bundle is not None}")
        if not line.strip() or self._bundle is None or self._busy:
            log(f"[DEBUG] early return")
            return
        log(f"[DEBUG] setting busy=True")
        self._busy = True
        log(f"[DEBUG] getting composer")
        composer = self.query_one("#composer", Input)
        log(f"[DEBUG] disabling composer")
        composer.disabled = True
        log(f"[DEBUG] calling _append_line")
        self._append_line(f"user> {line}")
        log(f"[DEBUG] calling _set_current_response")
        self._set_current_response("[dim]Working...[/dim]")
        log(f"[DEBUG] about to call handle_line...")
        log(f"[DEBUG] api_client type: {type(self._bundle.engine._api_client)}")
        log(f"[DEBUG] api_client base_url: {getattr(self._bundle.engine._api_client, 'base_url', 'N/A')}")
        try:
            ...
            log(f"[DEBUG] handle_line returned: {should_continue}")
    ...
    async def _print_system(self, message: str) -> None:
        ...
        log_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), "tui_debug.log")
        with open(log_path, "a") as f:
            f.write(f"[DEBUG] _print_system: {message}\n")
    ...
    async def _render_event(self, event: StreamEvent) -> None:
        ...
        def log(msg):
            with open(log_path, "a") as f:
                f.write(f"{msg}\n")
        log(f"[DEBUG] _render_event called: {type(event).__name__}")
