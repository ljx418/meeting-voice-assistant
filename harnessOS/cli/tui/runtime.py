...
from harnessOS.openharness.api.client import AnthropicApiClient, SupportsStreamingMessages
...
from harnessOS.openharness.api.provider import auth_status, detect_provider
from harnessOS.openharness.api.registry import detect_provider_from_registry
from harnessOS.openharness.bridge import get_bridge_manager
from harnessOS.openharness.commands import CommandContext, CommandResult, create_default_command_registry
from harnessOS.openharness.config import get_config_file_path, load_settings
from harnessOS.openharness.engine import QueryEngine
from harnessOS.openharness.engine.messages import (
    ConversationMessage,
    ToolResultBlock,
    ToolUseBlock,
    sanitize_conversation_messages,
)
from harnessOS.openharness.engine.query import MaxTurnsExceeded
from harnessOS.openharness.engine.stream_events import StreamEvent
from harnessOS.openharness.hooks import HookEvent, HookExecutionContext, HookExecutor, load_hook_registry
from harnessOS.openharness.hooks.hot_reload import HookReloader
from harnessOS.openharness.mcp.client import McpClientManager
from harnessOS.openharness.mcp.config import load_mcp_server_configs
from harnessOS.openharness.permissions import PermissionChecker
from harnessOS.openharness.plugins import load_plugins
from harnessOS.openharness.prompts import build_runtime_system_prompt
from harnessOS.openharness.state import AppState, AppStateStore
from harnessOS.openharness.services.session_backend import DEFAULT_SESSION_BACKEND, SessionBackend
from harnessOS.openharness.tools import ToolRegistry, create_default_tool_registry
from harnessOS.openharness.keybindings import load_keybindings
...
def _resolve_api_client_from_settings(settings) -> SupportsStreamingMessages:
    ...
    # Detect provider from the *original* settings (before materialization),
    # because materialize_active_profile() discards env-var overrides like
    # OPENHARNESS_MODEL / OPENHARNESS_API_FORMAT from .env files.
    ...
    # Fix api_format and base_url when the detected provider disagrees with
    # the materialized profile.  Without this, a DeepSeek model with the
    # default "claude-api" profile sends Anthropic-format requests.
    if spec is not None and spec.backend_type != settings.api_format:
        updates: dict[str, Any] = {"api_format": spec.backend_type}
    ...
    if settings.api_format == "copilot":
        from harnessOS.openharness.api.copilot_client import COPILOT_DEFAULT_MODEL
...
async def build_runtime(
    *,
    ...
    if settings.sandbox.enabled and settings.sandbox.backend == "docker":
        from harnessOS.openharness.sandbox.session import start_docker_sandbox
...
async def close_runtime(bundle: RuntimeBundle) -> None:
    ...
    from harnessOS.openharness.sandbox.session import stop_docker_sandbox

    await stop_docker_sandbox()
    # Extract local environment rules from session before closing
    try:
        from harnessOS.openharness.personalization.session_hook import update_rules_from_session
...
async def _render_command_result(
    ...
    if result.replay_messages and render_event is not None:
        ...
        from harnessOS.openharness.engine.stream_events import AssistantTextDelta, AssistantTurnComplete
