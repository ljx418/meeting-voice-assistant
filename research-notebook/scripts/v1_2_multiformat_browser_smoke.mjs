...
async function main() {
  ...
  try {
    ...
    for (const candidate of candidates) {
      if (!(await setFieldInForm(cdp, '导入来源', 0, candidate.title))) throw new Error(`could not set ${candidate.sourceType} source title`);
      if (!(await setFieldInForm(cdp, '导入来源', 1, candidate.sourceType))) throw new Error(`could not set ${candidate.sourceType} source type`);
      if (!(await setFieldInForm(cdp, '导入来源', 2, candidate.content))) throw new Error(`could not set ${candidate.sourceType} source content`);
      if (!(await submitForm(cdp, '导入来源'))) throw new Error(`could not submit ${candidate.sourceType} source`);
      ...
      await waitFor(() => textExists(cdp, '来源预览'), `${candidate.sourceType} drawer opens`);
      await waitFor(() => textExists(cdp, '文档单元'), `${candidate.sourceType} units visible`);
      await waitFor(
