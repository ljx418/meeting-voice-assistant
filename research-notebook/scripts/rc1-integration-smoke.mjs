...
const prefix = process.env.RN_RC_WORKSPACE_PREFIX ?? `rn-rc1-${Date.now()}`;
...
try {
  ...
  const sourceCreated = await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/sources`, {
    ...
    body: {
      texts: [
        {
          title: `${prefix} source`,
          content: 'ResearchNotebook RC1 validates source grounded ask, session workbench, graph context, and lightweight feedback.',
          metadata: { rc_stage: 'RC1' }
        }
      ],
      metadata: { rc_stage: 'RC1' }
    }
  ...
  const query = await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/query`, {
    method: 'POST',
    body: { query: 'What does the RC1 source validate?', top_k: 5 }
  });
  ...
  await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/sessions/${encodeURIComponent(sessionId)}/ingest`, {
    ...
    body: {
      ...
      content_format: 'text',
      source_type: 'text',
      content: 'Session RC1 context validates session ask with evidence fallback.'
    }
  });
  ...
  const sessionQuery = await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/sessions/${encodeURIComponent(sessionId)}/query`, {
    method: 'POST',
    body: { query: 'What does session RC1 validate?', top_k: 5 }
  });
  ...
  try {
    const graph = await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/graph/community`);
  ...
  if (workspaceId) {
    await request(`/api/workspaces/${encodeURIComponent(workspaceId)}/archive`, { method: 'POST', body: { reason: 'RC1 smoke cleanup' } });
  ...
  mark('rc1 smoke', 'fail', error.message);
